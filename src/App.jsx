...
function MainApp() {
  ...
  const css = `
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    .example-compare{display:grid;grid-template-columns:1fr;gap:14px;align-items:stretch;}
    .example-panel{display:flex;flex-direction:column;gap:10px;min-width:0;}
    .example-panel-label{font-family:'JetBrains Mono',monospace;font-size:8px;letter-spacing:.22em;text-transform:uppercase;color:var(--teal);}
    .after-panel .example-panel-label{color:var(--gold);}
    .before-text,.after-text{flex:1;font-family:'Cormorant Garamond',serif;font-size:clamp(22px,2.25vw,26px);line-height:1.58;border-radius:12px;padding:22px 22px;min-height:252px;display:flex;align-items:flex-start;color:var(--text);background:linear-gradient(145deg,rgba(10,10,18,.98),rgba(13,24,28,.78));border:1px solid #3ecfbe34;box-shadow:inset 0 0 0 1px rgba(255,255,255,.018);}
    .before-text{font-style:italic;border-left:3px solid var(--teal);}
    .after-text{border-left:3px solid var(--gold);}
    ...
    <div className="fade screen">
      <section className="hero-layout" aria-label="Presentazione ConText">
        ...
        <div className="example-card" aria-label="Esempio prima e dopo">
          ...
          <div className="example-compare" aria-label="Confronto tra messaggio originale e messaggio riscritto">
            <div className="example-panel before-panel">
              <div className="example-panel-label">Prima</div>
              <div className="before-text">“Non avete ancora pagato. È passato troppo tempo e questa cosa non va bene.”</div>
            </div>
            <div className="example-panel after-panel">
              <div className="example-panel-label">Dopo · tono professionale</div>
              <div className="after-text">“Vi scrivo per sollecitare gentilmente il saldo della fattura in sospeso. Resto disponibile per eventuali chiarimenti.”</div>
            </div>
          </div>
        </div>
      </section>
